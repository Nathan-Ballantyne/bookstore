...
func (m *UserModel) Insert(name, email, password string) error {
	...
	stmt := `INSERT INTO users (name, email, hashed_password, active)
	VALUES(?, ?, ?, true)`
...
func (m *UserModel) Get(id int) (*models.User, error) {
	...
	stmt := `SELECT id, name, email, active FROM users WHERE id = ?`
	err := m.DB.QueryRow(stmt, id).Scan(&u.ID, &u.Name, &u.Email, &u.Active)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
